...
- `bond`는 `dividend_representative`로 두고 `BND`의 최근 1년 배당수익률을 우선 참조합니다. 조회가 실패하면 기본 보정치 `2%p`를 fallback으로 사용합니다.
- `etf`는 표시명 `신성장주`로 노출되며 `equal_weight_basket`으로 동작합니다.

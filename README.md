...
이 프로젝트는 **관리자 유니버스에 등록된 섹터별 후보 종목군**을 바탕으로, 각 섹터에서 **대표 종목 1개씩**을 선택한 조합 중 Sharpe Ratio가 가장 우수한 구조를 찾고, 그 대표 종목 유니버스로 Efficient Frontier를 계산해 보여주는 FastAPI 데모입니다.

핵심은 두 단계입니다.

1. 섹터별 후보 종목군에서 대표 종목 1개씩을 고른다.
2. 그렇게 선택된 대표 종목 유니버스로 Efficient Frontier를 계산한다.

즉 현재 서비스는:

- 관리자는 `/admin`에서 섹터별 후보 종목을 관리합니다.
- 유니버스는 Postgres에 버전 단위로 저장되고 `active` 버전이 계산 기준이 됩니다.
- 가격 데이터는 `yfinance`로 적재합니다.
- 시뮬레이터는 active 유니버스의 **대표 종목 조합**을 먼저 고른 뒤, 그 종목들로 최적화를 수행합니다.
- 화면은 계산 결과를 섹터 기준으로 다시 합산해서 보여줍니다.
- 현재 메인 화면 입력은 `위험 슬라이더` 중심으로 단순화되어 있으며, 투자 기간과 직접 목표 변동성 입력은 숨겨져 있습니다.

## 현재 시스템 한 줄 요약

`섹터 후보군 관리 -> 가격 적재 -> 섹터별 대표 종목 1개 선택 -> 대표 종목 유니버스로 Efficient Frontier 계산 -> 섹터 기준 UI 표시`
...
1. `/admin`에서 섹터별 후보 종목을 등록합니다.
2. 저장된 유니버스 버전 중 하나를 `active`로 설정합니다.
3. active 유니버스 종목의 가격 이력을 `yfinance`로 적재합니다.
4. 가격 이력으로 종목별 수익률 행렬을 만듭니다.
5. 각 섹터에서 대표 종목 1개씩 선택하는 조합들을 만듭니다.
6. 조합 수가 작으면 전수 탐색하고, 크면 고정 seed 기반 샘플링을 수행합니다.
7. 각 조합에 대해 최대 Sharpe 포트폴리오를 계산합니다.
8. 가장 좋은 Sharpe를 만든 대표 종목 조합을 선택합니다.
9. 선택된 대표 종목 조합으로 Efficient Frontier 전체를 계산합니다.
10. 최종 결과를 섹터 비중과 섹터 리스크 기여도로 다시 합산해 화면에 보여줍니다.
...
- 종목 최대 비중: `50%`
- 평균 종목 상관관계 상한: `0.25`
- 전수 탐색 임계치: `5000` 조합 이하
- 샘플링 개수: `1000` 조합

즉 현재는 “섹터 후보군은 여러 개 둘 수 있지만, 실제 포트폴리오에는 섹터당 대표 종목 하나만 들어간다”는 구조입니다.
...
- **섹터 cap은 별도로 두지 않습니다.**
- 대신 각 섹터에서 대표 종목 1개만 선택합니다.
- 그래서 실제 포트폴리오 구성 종목 수는 섹터 수와 거의 동일합니다.
...
현재 구조는 “섹터 후보군 관리 + 대표 종목 선택 + Sharpe 기반 최적화” 단계입니다.
...
3. 관리자 보호장치 추가
4. 대표 종목 선택 규칙 고도화
5. 필요 시 섹터 cap / soft-penalty 제약 추가
